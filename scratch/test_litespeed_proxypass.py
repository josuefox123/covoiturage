...
rules_to_test = [
    ("PROXYPASS RULE", """DirectoryIndex disabled
ProxyPass /ws/ ws://127.0.0.1:8014/ws/
ProxyPassReverse /ws/ ws://127.0.0.1:8014/ws/
...
RewriteRule ^(.*)$ http://127.0.0.1:3001/$1 [P,L]
    ...
    ("REWRITE WITH ENV UPGRADE", """DirectoryIndex disabled
<IfModule mod_rewrite.c>
RewriteEngine On
RewriteBase /
RewriteRule ^(media|static)(/.*)?$ - [L]
RewriteCond %{HTTP:Upgrade} =websocket [NC]
RewriteRule ^ws/(.*)$ ws://127.0.0.1:8014/ws/$1 [P,L,E=HTTP_UPGRADE:websocket,E=HTTP_CONNECTION:Upgrade]
RewriteRule ^api/(.*)$ http://127.0.0.1:8014/api/$1 [P,L]
RewriteRule ^(.*)$ http://127.0.0.1:3001/$1 [P,L]
</IfModule>"""),

    ("DIRECT HTTP PROXY NO REWRITE ON UPGRADE", """DirectoryIndex disabled
<IfModule mod_rewrite.c>
RewriteEngine On
RewriteBase /
RewriteRule ^(media|static)(/.*)?$ - [L]
RewriteRule ^ws/(.*)$ http://127.0.0.1:8014/ws/$1 [P,L,E=HTTP_UPGRADE:%{HTTP:Upgrade},E=HTTP_CONNECTION:%{HTTP:Connection}]
RewriteRule ^api/(.*)$ http://127.0.0.1:8014/api/$1 [P,L]
RewriteRule ^(.*)$ http://127.0.0.1:3001/$1 [P,L]
</IfModule>""")
]
...
for name, ht_content in rules_to_test:
    print(f"\n--- TESTING {name} ---")
